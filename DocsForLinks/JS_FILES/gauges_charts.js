...
JustGage = function(config) {
  ...
  obj.config = {
    ...
    valueFontColor: kvLookup('valueFontColor', config, dataset, "#FCF3CF"),

    // valueFontFamily : string
    // color of label showing current value
    valueFontFamily: kvLookup('valueFontFamily', config, dataset, "Georgia"),
    ...
    gaugeColor: kvLookup('gaugeColor', config, dataset, "#EBDEF0"),
    ...
    labelFontColor: kvLookup('labelFontColor', config, dataset, "white"),

    // shadowOpacity : int
    // 0 ~ 1
    shadowOpacity: kvLookup('shadowOpacity', config, dataset, 1),
    ...
    shadowVerticalOffset: kvLookup('shadowVerticalOffset', config, dataset, 2),
    ...
    startAnimationTime: kvLookup('startAnimationTime', config, dataset, 500),
    ...
    donutStartAngle: kvLookup('donutStartAngle', config, dataset, 50),
    ...
    minLabelMinFontSize: kvLookup('minLabelMinFontSize', config, dataset, 20),

    // maxLabelMinFontSize
    // absolute minimum font size for the maximum label
    maxLabelMinFontSize: kvLookup('maxLabelMinFontSize', config, dataset, 20),
  ...
  if (obj.config.donut === true) {
    ...
    // title
    titleFontSize = ((widgetH / 8) > 10) ? (widgetH / 10) : 20;
    titleX = dx + widgetW / 2;
    ...
    // label
    labelFontSize = ((widgetH / 16) > 10) ? (widgetH / 16) : 100;
    labelX = dx + widgetW / 2;
    ...
    // title
    titleFontSize = ((widgetH / 8) > obj.config.titleMinFontSize) ? (widgetH / 20) : obj.config.titleMinFontSize;
    titleX = dx + widgetW / 2;
    titleY = dy + (obj.config.titlePosition === 'below' ? (widgetH * 1.07) : (widgetH / 6.4));

    // value
    valueFontSize = ((widgetH /  7.5) > obj.config.valueMinFontSize) ? (widgetH /  7.5) : obj.config.valueMinFontSize;
    valueX = dx + widgetW / 2;
    valueY = dy + widgetH / 1.275;

    // label
    labelFontSize = ((widgetH / 10) > obj.config.labelMinFontSize) ? (widgetH / 10) : obj.config.labelMinFontSize;
    labelX = dx + widgetW / 2;
    labelY = valueY + valueFontSize / 2 + 5;

    // min
    minFontSize = ((widgetH / 5) > obj.config.minLabelMinFontSize) ? (widgetH / 10) : obj.config.minLabelMinFontSize;
    minX = dx + (widgetW / 10) + (widgetW / 6.666666666666667 * obj.config.gaugeWidthScale) / 2;
    minY = labelY;

    // max
    maxFontSize = ((widgetH / 5) > obj.config.maxLabelMinFontSize) ? (widgetH / 10) : obj.config.maxLabelMinFontSize;
    maxX = dx + widgetW - (widgetW / 10) - (widgetW / 6.666666666666667 * obj.config.gaugeWidthScale) / 2;
    maxY = labelY;
  }
  ...
  obj.txtMax.attr({
    ...
    "font-family": "Georgia",
    "fill": obj.config.labelFontColor,
    "fill-opacity": (obj.config.hideMinMax || obj.config.donut) ? "1" : "1"
  });
